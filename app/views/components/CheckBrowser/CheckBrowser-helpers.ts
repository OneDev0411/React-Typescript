...
export function browserStatus(): BrowserStatus{
    ...
    const version = browserInfo.version ? Number(browserInfo.version.split('.')[0]) : 0
...
export function generatePropeprMessage(status:BrowserStatus){
    ...
    if(!status.isSupported){
        output.text = 'For the best experience on Rechat, we recommend using Google Chrome.'
    ...
    if(status.isOutdated === true){
        ...
        output.actionLink = status.meta.browserName === 'Safari' ? downloadLink.safari : downloadLink.chrome
    }
