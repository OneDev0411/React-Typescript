...
export default class MessagesList extends Component {
  ...
  getLockedHeadingDate() {
    ...
    if (!heading_objects)
      return
    const arr_sorted = heading_objects.sort((a, b) => a.top - b.top)
    const sorted_under_zero = arr_sorted.filter(message => message.top < 60)
        ...
        if (!prev_message_date || prev_message_date && prev_message_date !== message_date) {
          ...
          heading = (
            <div className="message-heading" style={heading_style}>
              <div style={S('absolute w-100p h-2 bg-e8e8e8 t-12 z-0')} />
