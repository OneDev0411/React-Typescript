  ...
  render() {
    ...
    if (message.attachments.length) {
      ...
      attachments.forEach((attachment) => {
        ...
        if (ext.indexOf('png') !== -1 || ext.indexOf('jpg') !== -1 || ext.indexOf('gif') !== -1) {
          message_thumb = (
            <div style={S(`${message_thumb_size} br-3 bg-url(${file_url}) no-repeat bg-cover mb-10`)} />
          )
        } else {
          message_thumb = (
            <div style={S(message_thumb_size)}>
              <i style={S('font-150')} className="text-primary fa fa-file-o" />
              <br />
        ...
        return (
          <div style={S('relative mb-5 font-15')}>
            <div className="pull-left" style={S('ml-55')}>
              <div className={message_class_name} dangerouslySetInnerHTML={{ __html: message_text }} />
              { message_image }
            </div>
    ...
    if (recommendation) {
      ...
      if (listing && listing.cover_image_url) {
        ...
        message_image = (
          <div style={S(`h-200 bg-url(${cover_image_url}) bg-cover bg-center ${card_width}`)} />
        )
      }
      ...
      profile_image_div = (
        <ProfileImage data={data} user={author || null} />
      )
      if (!author) {
        profile_image_div = (
          <div style={S('absolute w-35 t-5')}>
      ...
      // If text comment
      let message_area = (
        <div style={S('mt-5 pull-left ml-10 w-80p mt-10 font-15')} className={message_class_name} dangerouslySetInnerHTML={{ __html: `"${message_text}"` }} />
      )
      // If image comment
      if (!message_text && message.attachments && message.attachments.length) {
        ...
        message_area = (
          <div style={S('mt-5 pull-left ml-10 w-80p mt-10 font-15')} className={message_class_name}>
            ...
            <div onClick={this.props.showFileViewer.bind(this, attachment)} style={S(`bg-url(${image_url}) bg-cover w-80p h-200 mb-10`)} />
          </div>
        )
      }
