...
export default class TransactionDetail extends Component {
  ...
  render() {
    ...
    if (listing) {
      listing_images = (
        <Carousel interval={0} indicators={false} prevIcon={prev_icon} nextIcon={next_icon}>
          <CarouselItem>
            <div style={S(`${carousel_wh} bg-cover bg-center bg-url(${listing.cover_image_url})`)} />
          </CarouselItem>
          {
            listing.gallery_image_urls.map(gallery_image_url => (
              <CarouselItem key={`gallery-image-${gallery_image_url}`}>
                ...
                <div style={S(`${carousel_wh} bg-cover bg-center bg-url(${gallery_image_url})`)} />
              </CarouselItem>
              ))
          }
        </Carousel>
    ...
    if (data.show_document_modal) {
      ...
      current_file_image = <div className="center-block" style={S('w-100 h-100 p-15')}><i style={S('font-60 color-929292')} className="fa fa-file-o" /></div>
    ...
    if (listing && listing.cover_image_url) {
      listing_image = (
        <div style={S(`absolute w-100p h-100p bg-center bg-cover bg-url(${listing.cover_image_url})`)} />
      )
    } else {
      listing_image = (
            ...
            <div className="pull-left text-center">
              <Dropdown id="settings-dropdown">
                <Dropdown.Toggle noCaret style={S('pointer bg-F7F9FA relative t-7 br-100 p-8 w-35 h-35 bw-0')}>
                  <i style={S('color-929292')} className="fa fa-cog" />
                </Dropdown.Toggle>
                <Dropdown.Menu className="super-colors">
                  <MenuItem onClick={this.props.showEditModal.bind(this)}><i className="fa fa-edit" style={S('mr-10')} />Edit</MenuItem>
                  <MenuItem divider />
                  <MenuItem onClick={this.props.deleteTransaction.bind(this, transaction.id)}><i className="fa fa-trash-o" style={S('mr-10')} />{ data.deleting_transaction && data.deleting_transaction === transaction.id ? 'Deleting...' : 'Delete' }</MenuItem>
                </Dropdown.Menu>
              </Dropdown>
            </div>
