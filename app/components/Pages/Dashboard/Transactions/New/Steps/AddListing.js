...
export default class AddListing extends Component {
  ...
  render() {
    ...
    if (listing_added || listing_data) {
      ...
      listing_added_markup = (
        <div style={S('h-50 mt-30 relative br-100 p-3 pl-0 pr-10 mb-10 mr-10 w-500')} className="pull-left">
          <div style={S(`w-25 h-25 bg-cover bg-url(${cover_image_url}) l-0 t-0 absolute br-100`)} />
          <div style={S('ml-50')}>
    ...
    if (listing_added && listing_added.cover_image_url) {
      listing_image = (
        <div style={S(`absolute w-100p h-100p bg-center bg-cover bg-url(${listing_added.cover_image_url})`)} />
      )
    } else {
      listing_image = (
