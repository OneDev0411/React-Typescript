...
describe('reducer', () => {
  ...
  it('should change result info for only total property', () => {
    const action: Actions = {
    ...
    const expected: ListingsState = {
      ...
      result: {
        ...initialState.result,
        info: { ...initialState.result.info, total: 195 }
      }
    }

    expect(actual).toEqual(expected)
  })
  it('should change result info for only total and count properties', () => {
    const action: Actions = {
      type: 'CHANGE_RESULT_INFO',
    ...
    const expected: ListingsState = {
      ...
      result: {
        ...initialState.result,
        info: { ...initialState.result.info, total: 1950, count: 12 }
      }
    }

    expect(actual).toEqual(expected)
  })
  it('should change result info for only all properties', () => {
    const action: Actions = {
      type: 'CHANGE_RESULT_INFO',
      payload: {
        info: { count: 4, total: 200, proposed_title: 'some title' }
      }
    }
    const actual = reducer(initialState, action)
    const expected: ListingsState = {
