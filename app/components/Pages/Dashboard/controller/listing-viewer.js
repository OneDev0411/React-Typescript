...
const controller = {
  ...
  handleModalGalleryNav(selectedIndex, selectedDirection) {
    ...
    if (selectedDirection === 'prev') { AppStore.data.modal_gallery.current_index = current_index - 1 }
    if (selectedDirection === 'next') { AppStore.data.modal_gallery.current_index = current_index + 1 }
    if (AppStore.data.modal_gallery.current_index === -1) { AppStore.data.modal_gallery.current_index = gallery_image_urls.length - 1 }
    if (AppStore.data.modal_gallery.current_index === gallery_image_urls.length) { AppStore.data.modal_gallery.current_index = 0 }
