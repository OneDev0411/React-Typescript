  ...
  render() {
    ...
    const zoom_controls = (
      <ButtonGroup className="transition" vertical style={S(`fixed ${zoom_right}${zoom_bottom}`)}>
        <Button bsSize="large" onClick={controller.listing_map.handleZoomClick.bind(this, 'in')}><i style={S('color-929292')} className="fa fa-plus" /></Button>
        <Button bsSize="large" onClick={controller.listing_map.handleZoomClick.bind(this, 'out')}><i style={S('color-929292')} className="fa fa-minus" /></Button>
      </ButtonGroup>
    )
    let results_actions
          ...
          <div style={alert_header_style}>
            ...
            <div style={S('relative ml-15 mt-10 color-fff z-1 font-15')}>
              ...
              <div style={S('pull-right pointer')} onClick={controller.alert_map.showAlertViewer.bind(this)}>
                ...
                <span className={!current_alert.feed || (current_alert.feed && !current_alert.feed.length) ? 'hidden' : ''} style={S('mr-15 relative t-2')}><i style={S('color-98caf1')} className="fa fa-chevron-right" /></span>
              </div>
            </div>
          </div>
        )
