...
export default class Mls extends Component {
  componentWillMount() {
    ...
    // Allow for seamless
    // if (
    //     (!AppStore.data.mounted || AppStore.data.mounted) && AppStore.data.mounted.indexOf('recents') === -1
    // ) {
    ...
    if (
      ...
      location.query.message === 'welcome'
    ) {
  ...
  render() {
    ...
    const marginLeft = user ? 'l-70' : 'l-0'
    let main_style = S(`absolute h-100p ${marginLeft}`)
    ...
    if (listing_map && listing_map.is_loading) {
      ...
      loading = (
        <div id="loading" style={loading_style}>
          <div
            style={S(`br-20 color-fff w-190 h-29 pt-5 center-block text-center bg-${Brand.color('primary', '3388ff')}`)}
          >
            Loading MLS&reg; Listings...
          </div>
        </div>
      )
    }
    ...
    if (data.show_listing_viewer) {
      listing_viewer = (
        <ListingViewer
          ...
          hideListingViewer={
            controller.listing_viewer.hideListingViewer.bind(this)
          }
    ...
    if (data.show_search_map) {
      ...
      create_alert_button = (
        <Button
          style={save_search_btn_style}
          onClick={controller.alert_share.handleAlertShareClick.bind(this)}
        >Save Search</Button>
      )
    }
    ...
    if (data.show_signup_form) {
      ...
      signup_form = (
        <div style={signup_form_style}>
          <div
            className="close"
            style={S('absolute r-15 t-10')}
            onClick={this.handleCloseSignupForm}
          >&times;</div>

          <div
            className="din"
            style={S('font-30 color-263445 mb-5')}
          >Get the Fastest Listing Alerts</div>

          <div style={S('mb-5 w-100p')}>
            <form
              onSubmit={this.handleEmailSubmit.bind(this)}
              style={S('mb-5 center-block pull-left w-100p block')}
            >
              <div style={S('pull-left')}>
                ...
                >
                  <FormControl
                    ...
                    placeholder="Enter email to save this search"
                  />
                </OverlayTrigger>
              </div>
              <div style={S('pull-left')}>
                <Button
                  type="submit"
                  className={data.submitting ? 'disabled' : ''} bsStyle="primary" style={signup_btn_style}
                >
    ...
    if (listing_map && listing_map.listings) {
      ...
      if (
        data.listing_panel &&
        data.listing_panel.view === 'photos' &&
        data.listing_panel.size === 'full'
      ) {
        ...
        <div>
          ...
          <ButtonGroup style={viewing_type_button_group_style}>
            <Button
              style={btn_style}
              onClick={
                controller.listing_panel
                  .hideListingPanel.bind(this)
              }
            >
              <div style={S('mt-3')}>
            ...
            <Button
              ...
              onClick={
                controller.listing_panel
                  .showPanelView.bind(this, 'photos', 'full')
              }
            >
              <div style={S('mt-3')}>
    ...
    if (search_input && search_input.listings) {
      ...
      listing_list = listings.map((listing, i) => {
        ...
        let cover_image = (
          <div
            style={S('bg-929292 w-87 h-50 color-fff text-center pt-15')}
          >
            No Image
          </div>
        )
        ...
        if (listing.cover_image_url) {
          cover_image = (
            <div style={S(`w-87 h-50 bg-cover bg-center bg-url(${listing.cover_image_url})`)} />
          )
        }
        ...
        return (
          <div
            key={listing.id}
            className="search-listings__listing"
            style={S(`br-3 h-62 relative pointer p-5 ${bg_color}`)}
            onClick={
              controller.listing_viewer.showListingViewer.bind(this, listing)
            }
          >
            ...
            <div style={S('mt-5')} className="pull-left">
              <span style={S('color-666')}>
                <span style={S('mr-10')}>
                  <b>{listing.address.street_number } { listing.address.street_name } { listing.address.street_suffix }</b>
                </span>
                <span style={S('mr-10 font-12 color-929292')}>
                  <span
                    style={
                      S(`font-20 t-5 absolute color-${listing_util.getStatusColor(listing.status)}`)
                    }
                  >
                    &#8226;
                  </span>
                ...
                <br />
                <span style={S('color-929292 font-10')}>
    ...
    if (listing_list) {
      listing_area = (
        <div style={{ overflow: 'scroll', ...S('w-504 bg-fff br-3 maxh-250 z-1 relative t-5') }}>
          {listing_list}
        </div>
      )
    }
    ...
    const options_gist = (
      <div style={S('relative mt-10 w-504')}>
        <div style={S('z-0 op-.4 bg-000 absolute w-504 h-40 br-3')} />
        <div style={S('z-1 color-fff p-10 relative w-504')}>
          { options_text }
        </div>
      </div>
    )
      ...
      <div style={S('relative t-35 l-10 z-1 w-580')}>
        <div style={search_area_style}>
          <div style={S('pull-left mr-10 relative')}>
            ...
            <img
              style={S('absolute r-15 t-15 w-20 pointer')}
              onClick={
                controller.search_input_map.handleSearchSubmit.bind(this)
              }
              src="/static/images/dashboard/mls/search.svg"
            />

            <div style={S('w-1 h-28 absolute l-400 t-13 bg-dddddd')} />
          </div>

          <div style={S('pull-left')}>
            <Button
              id="open_filters"
              style={{ ...S('h-50 border-1-solid-fff pt-15'), outline: 'none' }}
              onClick={
                controller.listing_filter.showFilterForm.bind(this, 'photos')
              }
            >
        ...
        <div style={S('pull-left')}>
          <Button
            ...
            onClick={controller.listing_map.toggleDrawable.bind(this)}
          >
    ...
    const underline = <div style={S(`w-100p h-6 bg-${Brand.color('primary', '3388ff')} absolute b-11n`)} />

    const map_tabs = (
      <ul style={S('relative l-30n t-5')}>
        <li style={S('relative pull-left font-28 mr-60')}>
          <span
            onClick={this.handleTabClick.bind(this, 'search')}
            style={S(`pointer ${data.show_search_map ? 'color-263445' : 'color-8696a4'}`)}
          >Search</span>
        ...
        <li style={S('relative pull-left color-263445 font-28 mr-60')}>
          <span
            onClick={this.handleTabClick.bind(this, 'alerts')}
            style={S(`pointer ${data.show_alerts_map ? 'color-263445' : 'color-8696a4'}`)}
          >Saved Searches</span>
        ...
        <li style={S('relative pull-left color-263445 font-28 mr-60')}>
          <span
            onClick={this.handleTabClick.bind(this, 'actives')}
            style={S(`pointer ${data.show_actives_map ? 'color-263445' : 'color-8696a4'}`)}
          >Saved Listings</span>
    ...
    let toolbar = (
      <nav style={toolbar_style}>
        <div style={!data.is_widget ? S('h-45 mt-10n') : {}}>
          { user && !data.is_widget ? map_tabs : '' }
        </div>
        <div className="clearfix" />
        { search_filter_draw_area }
        { results_actions }
    ...
    if (data.is_mobile) {
      search_area = (
        <form
          onSubmit={controller.search_input_map.handleSearchSubmit.bind(this)}
        >
          <img
            src="/static/images/dashboard/mls/search.svg"
            style={S('pointer w-22 h-22 absolute l-13 t-14')}
            onClick={controller.search_input_map.handleSearchSubmit.bind(this)}
          />
          <input
            value={search_input_text}
      ...
      toolbar = (
        <nav style={S('border-bottom-1-solid-ccc p-5 fixed t-0 w-100p z-10 bg-fff')}>
          ...
          <div style={S('pull-right')}>
            <Button
              onClick={controller.listing_map.toggleDrawable.bind(this)}
              style={{ ...S('mr-10'), outline: 'none' }}
            >
              <img
                style={S('w-20')}
                src={`/static/images/dashboard/mls/draw${data.listing_map && data.listing_map.drawable ? '-active' : ''}.svg`}
              />
            </Button>
            ...
            >
              <img
                src={`/static/images/dashboard/mls/filters${data.show_filter_form ? '-active' : ''}.svg`}
                style={S('w-20 mr-10')}
              />
              <span className={data.show_filter_form ? 'text-primary' : ''}>Filters</span>
            </Button>
        ...
        alert_header_area = (
          <div style={alert_header_style}>
            ...
            <div style={S('relative ml-15 mt-10 color-fff z-1 font-15')}>
              ...
              >
                <span
                  style={S('color-98caf1 mr-15')}
                >{ new_listings_link }</span>
                <span
                  className={!current_alert.feed || (current_alert.feed && !current_alert.feed.length) ? 'hidden' : ''}
                  style={S('mr-15 relative t-2')}
                >
                  <i
                    style={S('color-98caf1')} className="fa fa-chevron-right"
                  />
                </span>
              </div>
            </div>
    ...
    if (listing_map && listing_map.show_share_alert_error_tooltip && !data.current_listing) {
      ...
      share_alert_error_tooltip = (
        <div style={warning_style}>
          ...
          <div
            onClick={controller.listing_map.hideModal}
            style={S('font-17 color-9b9b9b pointer')}
          >Close</div>
        </div>
      )
    }
    ...
    if (!data.user) {
      signup_message = (
        <div style={S(`absolute ${data.is_mobile ? 'b-20 l-10' : 'b-50 l-20'} color-fff`)}>
          <div style={S('absolute bg-000 op-.5 br-3 h-36 w-100p z-0')} />
          <div style={S('z-1 relative pt-8 text-center pl-15 pr-15')}>
            <a
              style={{ textDecoration: 'underline', ...S('color-fff') }}
              href="/signup"
            >Sign up</a> or <a style={{ textDecoration: 'underline', ...S('color-fff') }} href="/signin">log in</a> to save listings and chat with our agents.</div>
        </div>
      )
    }
    ...
    if (data.widget || data.brand) {
      powered_by = (
        <a
          ...
          style={S(`font-12 w-150 h-30 text-center absolute ${data.is_mobile ? 'b-55 l-10' : 'b-50 r-100'} color-fff bg-2196f3 br-3 pt-7`)}
        >
    ...
    const main_content = (
      <main>
        ...
        <FilterForm
          ...
          changeSchoolsSelected={
            controller.listing_filter.changeSchoolsSelected
          }
          changeSubAreasSelected={
            controller.listing_filter.changeSubAreasSelected
          }
          changeCountiesSelected={
            controller.listing_filter.changeCountiesSelected
          }
          showSchoolDistrictsList={
            controller.listing_filter.showSchoolDistrictsList
          }
          changeHomeStylesSelected={
            controller.listing_filter.changeHomeStylesSelected
          }
          changeSubdivisionsSelected={
            controller.listing_filter.changeSubdivisionsSelected
          }
          toggleListingStatusDropdown={
            controller.listing_filter.toggleListingStatusDropdown
          }
          changeSchoolDistrictsSelected={
            controller.listing_filter.changeSchoolDistrictsSelected
          }
          handleFilterStatusOptionSelect={
            controller.listing_filter.handleFilterStatusOptionSelect
          }
        />

        {
          listing_map && listing_map.show_share_type_modal
          ? share_type_modal_area : ''
        }

        {
          listing_map && listing_map.show_share_alert_error_tooltip
          ? share_alert_error_tooltip : ''
        }
        ...
        <ShareAlertModal
          ...
          handlePhoneNumberChange={
            controller.share_modal.handlePhoneNumberChange
          }
        />

        <Modal
          dialogClassName="modal-alert-saved"
          show={data.show_alert_saved_modal}
          onHide={controller.listing_map.hideModal}
        >
        ...
        <Modal
          dialogClassName="modal-alert-saved"
          show={data.show_listing_shared_modal}
          onHide={controller.listing_map.hideModal}
        >
          <div className="din" style={S('text-center font-60 color-fff')}>
            <div style={S('bg-2196f3 w-165 h-165 br-100 center-block pt-35')}>
    ...
    return (
      <div style={S('minw-1000')}>
        ...
        <Modal
          ...
          show={data.show_welcome_modal}
          onHide={this.hideWelcomeModal}
        >
          <Modal.Body style={S('text-center p-50')}>
            ...
            <div style={S('mt-20 mb-20 h-156')}>
              <img
                style={S('w-100p')}
                src="/static/images/signup/value-faces.png"
              />
            </div>
            <Button
              bsStyle="primary"
        ...
        <Modal
          dialogClassName={data.is_mobile ? 'modal-mobile' : ''}
          show={data.show_filter_error_modal}
          onHide={this.hideFilterErrorModal}
        >
          <Modal.Body style={S('text-center p-50')}>
            <div
              className="close"
              onClick={this.hideFilterErrorModal}
              style={S('absolute t-15 r-15 font-26')}
            >&times;</div>
