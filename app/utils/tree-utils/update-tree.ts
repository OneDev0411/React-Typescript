...
export function updateTree<T extends Tree>(tree: T, predicate, updater): T {
  const [searchNode, ...path] = bfs<Tree>(
    parent => (parent ? parent.children || [] : [tree]),
    (node, parents) => {
  ...
  return searchNode
    ? getUpdatedTree([...[...path].reverse(), searchNode], updater(searchNode))
    : tree
}
...
function getUpdatedTree(path: Tree[], newChild: Tree) {
  ...
  return {
    ...parent,
    children: parent.children!.map(aChild =>
      aChild === rest[0] ? getUpdatedTree(rest, newChild) : aChild
    )
  }
}
