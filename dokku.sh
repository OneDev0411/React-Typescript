...
[[ -z $DOKKU_SOURCE_APP ]] || REVIEW_SOURCE_APP=$DOKKU_SOURCE_APP

echo "Source App: $DOKKU_SOURCE_APP, $REVIEW_SOURCE_APP"
...
ssh "dokku@$REVIEW_HOST" letsencrypt:list | grep $APP
if [ $? -eq 0 ]; then
